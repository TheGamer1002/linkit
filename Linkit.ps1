...
$folder = $args[0]; # The folder to move
Write-Output $folder # for debugging purposes
    ...
    if ($null -eq $config) {
        ...
        if ($result) {
            ...
            try {
                ...
                exit # just exit the script. Don't want to break anything more
            }
                ...
                if (Test-Path -Path $dest -PathType Container) {
                    ...
                    if (Get-ChildItem -Path $dest -Recurse -Force -ErrorAction SilentlyContinue) { # Destination is not empty
                        ...
                        if ($result) { # User chose to create link anyway
                            ...
                            # Exit
                            Get-error "User chose not to create the link"
                            exit
                        } #END prompt for confirmation due to non-empty destination
